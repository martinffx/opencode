...
describe("session.llm.stream", () => {
  ...
  const anthropicFixture = { providerID: "anthropic", modelID: "claude-opus-4-6" }
  it.instance(
    "sends split system as two messages for Anthropic-compatible models",
    () =>
      Effect.gen(function* () {
        const model = loadFixture(anthropicFixture.providerID, anthropicFixture.modelID).model
        const chunks = [
          {
            type: "message_start",
            message: {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderID.make(anthropicFixture.providerID),
          ModelID.make(model.id),
        )
        ...
        const user = {
          ...
          time: { created: Date.now() },
          agent: agent.name,
          model: { providerID: ProviderID.make(anthropicFixture.providerID), modelID: resolved.id },
        } satisfies MessageV2.User
    ...
    {
      config: () => ({
        enabled_providers: [anthropicFixture.providerID],
        provider: {
          [anthropicFixture.providerID]: {
            name: "Anthropic",
            env: ["ANTHROPIC_API_KEY"],
            npm: "@ai-sdk/anthropic",
            api: "https://api.anthropic.com/v1",
            models: { [anthropicFixture.modelID]: configModel(loadFixture(anthropicFixture.providerID, anthropicFixture.modelID).model) as ConfigModel },
            options: { apiKey: "test-anthropic-key", baseURL: `${state.server!.url.origin}/v1` },
          },
        },
      }),
  ...
  it.instance(
    "sends flat system as single message for Anthropic-compatible models",
    () =>
      Effect.gen(function* () {
        const model = loadFixture(anthropicFixture.providerID, anthropicFixture.modelID).model
        const chunks = [
